...
class PropertyIntrospectionTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testInvalidConstructorArguments()
    {
        ...
        new PropertyIntrospection(self::TEST_CLASS, 'thisPropertyDoesNotExist');
    }

    public function testExport()
    {
        foreach (self::TEST_NAMES as $property) {
            $export = PropertyIntrospection::export(self::TEST_CLASS, $property);
        ...
        $export = PropertyIntrospection::export($class, 'dynamic_property');
        $this->assertSame('Property [ <implicit> $dynamic_property ]', $export);

        $this->expectException('SR\Exception\InvalidArgumentException');
        PropertyIntrospection::export(self::TEST_CLASS, 'thisPropertyDoesNotExist');
    }

    public function testDeclaringClass()
    {
        $inspect = new PropertyIntrospection(self::TEST_CLASS, self::TEST_NAME);
    ...
    {
        foreach (self::TEST_NAMES as $property) {
            $inspect = new PropertyIntrospection(self::TEST_CLASS, $property);
    ...
    {
        foreach (self::TEST_NAMES as $property) {
            $_ = new PropertyIntrospection(self::TEST_CLASS, $property);
